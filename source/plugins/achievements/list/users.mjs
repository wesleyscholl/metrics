...
export default async function({list, login, data, computed, imports, graphql, queries, rest, rank, leaderboard}) {
  ...
  //Developer
  {
    ...
    list.push({
      title:"Developer",
      text:`Published ${devValue} public repositor${imports.s(devValue, "y")}`,
      ...
      ...rank(devValue, [1, 20, 50, 100, 250]),
      value:devValue,
      unlock:safeDate(devUnlock?.createdAt)
   })
  }
  ...
  //Follower
  {
    ...
    list.push({
      title:"Follower",
      text:`Followed by ${followerValue} user${imports.s(followerValue)}`,
      ...
      ...rank(followerValue, [1, 10, 50, 100, 500]),
      value:followerValue,
      unlock:safeDate(followerUnlock?.createdAt)
    })
  }
